class Profiler
    ...
    def print_results_call_tree(out_file_id)
      ...
      File.open "/tmp/callgrind.wall.p#{pct_str_for_fname}.#{out_file_id}", 'w' do |file|
        RubyProf::CallTreePrinter.new(@results).print(file, :min_percent => min_percent)
      end
    end

    def print_results_call_stack(out_file_id)
      File.open "/tmp//#{out_file_id}-stack.html", 'w' do |file|
        RubyProf::CallStackPrinter.new(@results).print(file)
      end
    end

    def print_results_flat(out_file_id)
      File.open "/tmp//#{out_file_id}-flat.txt", 'w' do |file|
        RubyProf::FlatPrinterWithLineNumbers.new(@results).print(file)
      end
    end

    def print_results_graph_html(out_file_id)
      File.open "/tmp/#{out_file_id}-graph.html", 'w' do |file|
